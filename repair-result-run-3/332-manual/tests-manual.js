const schorschInitialization = async function (t) {
    ...
    let schorsch = t.getSprites(sprite => sprite.name.includes('Schorsch'))[0];
...
const stellaInitialization = async function (t) {
    await t.runForSteps(5);
    let stella = t.getSprites(sprite => sprite.name.includes('Stella'))[0];
...
const schorschDrawing = async function (t) {
    ...
    t.onSpriteMoved(() => {
        ...
        const tolerance = 10e-12;
        if (Math.abs(xOrigSchorsch - schorsch.x) > tolerance || Math.abs(yOrigSchorsch - schorsch.y) > tolerance) {
            schorschMoved = true;
        }
    });
    await t.runUntil(() => stellaFinished, 16000);
