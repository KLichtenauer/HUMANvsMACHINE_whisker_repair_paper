...
const testMoveBowlDetails = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testMoveBowl(t, true) --------------------------------
    ...
    // -------------------------------- END INLINED CALL to testMoveBowl(t, true) --------------------------------
};
...
const testAppleFallingDetails = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testFruitFalling(t, isApple, 'Apple', -5, true) --------------------------------
    ...
    /* Wait until a fruit shows at the top of the screen. */
    let fruit = await waitForSpriteOnTop(t, isApple, 5000);
    t.assume.ok(fruit !== null, 'Apple', ' must appear on the top of the screen after 5 seconds.');
    ...
    t.addConstraint(() => {
        if (fruit.y !== fruit.old.y) {
            t.assert.lessOrEqual(fruit.y, fruit.old.y, 'Apple', ' must not move upwards.');
            t.assert.ok((fruit.old.y - fruit.y) % -5 === 0,
                'Apple', ' must fall with a speed of ', -5, '.');
            t.assert.equal(fruit.x, fruitPos.x, `Apple must fall in a straight line.`);
        }
    });
    ...
    /* Check if the fruit moved down.
     * Use fruit.old.y because the fruit could have already been moved to the top again. */
    t.assume.less(fruit.old.y, fruitPos.y, 'Apple', ' must fall down.');

    t.end();

    // -------------------------------- END INLINED CALL to testFruitFalling(t, isApple, 'Apple', -5, true) --------------------------------
};

const testBananaFalling = async function (t) {
    // await testFruitFalling(t, isBanana, 'Banana', -7, false);

    // -------------------------------- BEGIN INLINED CALL to testFruitFalling(t, isBanana, 'Banana', -7, false) --------------------------------
    ...
    /* Wait until a fruit shows at the top of the screen. */
    let fruit = await waitForSpriteOnTop(t, isBanana, 5000);
    t.assert.ok(fruit !== null, "Banana", ' must appear on the top of the screen after 5 seconds.');
    ...
    /* Check if the fruit moved down.
     * Use fruit.old.y because the fruit could have already been moved to the top again. */
    t.assert.less(fruit.old.y, fruitPos.y, "Banana", ' must fall down.');

    t.end();

    // -------------------------------- END INLINED CALL to testFruitFalling(t, isBanana, 'Banana', -7, false) --------------------------------
};

const testBananaFallingDetails = async function (t) {
    // await testFruitFalling(t, isBanana, 'Banana', -7, true);

    // -------------------------------- BEGIN INLINED CALL to testFruitFalling(t, isBanana, 'Banana', -7, true) --------------------------------
    ...
    /* Wait until a fruit shows at the top of the screen. */
    let fruit = await waitForSpriteOnTop(t, isBanana, 5000);
    t.assume.ok(fruit !== null, 'Banana', ' must appear on the top of the screen after 5 seconds.');
    ...
    t.addConstraint(() => {
        if (fruit.y !== fruit.old.y) {
            t.assert.lessOrEqual(fruit.y, fruit.old.y, 'Banana', ' must not move upwards.');
            t.assert.ok((fruit.old.y - fruit.y) % -7 === 0,
                'Banana', ' must fall with a speed of ', -7, '.');
            t.assert.equal(fruit.x, fruitPos.x, `Banana must fall in a straight line.`);
        }
    });
    ...
    /* Check if the fruit moved down.
     * Use fruit.old.y because the fruit could have already been moved to the top again. */
    t.assume.less(fruit.old.y, fruitPos.y, 'Banana', ' must fall down.');

    t.end();

    // -------------------------------- END INLINED CALL to testFruitFalling(t, isBanana, 'Banana', -7, true) --------------------------------
};
...
const testAppleSpawnRandomXPosition = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testFruitSpawn(t, isApple, 'Apple', true) --------------------------------
    ...
    t.addCallback(() => {
        ...
        for (const fruit of fruits) {
            if (oldFruits.indexOf(fruit) === -1) {
                appleSpawnPositions.push(fruit.pos);
            }
        }
    ...
    await t.runUntil(() => appleSpawnPositions.length >= 3, 30000);

    t.assume.greaterOrEqual(appleSpawnPositions.length, 3,
        'At least three ', 'Apple', 's must have spawned after 30 seconds.');

    t.end();

    /* Ignore the first spawn position because it might be caused by an uninitialized sprite falling down. */
    appleSpawnPositions.shift();

    // -------------------------------- END INLINED CALL to testFruitSpawn(t, isApple, 'Apple', true) --------------------------------
...
const testBananaSpawn = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testFruitSpawn(t, isBanana, 'Banana', false) --------------------------------
    ...
    let oldFruits = t.getSprites(sprite => sprite.visible && isBanana(sprite) && sprite.y > 100);
    const fruitSpawnPositions = [];

    /* Detect fruit spawns by checking if any new fruit appears on the top of the screen each step.
     * Save the spawn position for detected fruits. */
    t.addCallback(() => {
        let fruits = t.getSprites(sprite => isBanana(sprite) && sprite.visible && sprite.y > 100);
    ...
    t.assert.greaterOrEqual(fruitSpawnPositions.length, 3,
        'At least three ', 'Banana', 's must have spawned after 30 seconds.');

    t.end();

    // -------------------------------- END INLINED CALL to testFruitSpawn(t, isBanana, 'Banana', false) --------------------------------
};
...
const testBananaSpawnRandomXPosition = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testFruitSpawn(t, isBanana, 'Banana', true) --------------------------------
    ...
    t.addCallback(() => {
        ...
        for (const fruit of fruits) {
            if (oldFruits.indexOf(fruit) === -1) {
                bananaSpawnPositions.push(fruit.pos);
            }
        }
    ...
    await t.runUntil(() => bananaSpawnPositions.length >= 3, 30000);

    t.assume.greaterOrEqual(bananaSpawnPositions.length, 3,
        'At least three ', 'Banana', 's must have spawned after 30 seconds.');

    t.end();

    /* Ignore the first spawn position because it might be caused by an uninitialized sprite falling down. */
    bananaSpawnPositions.shift();

    // -------------------------------- END INLINED CALL to testFruitSpawn(t, isBanana, 'Banana', true) --------------------------------
...
const testOnlyOneApple = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testOnlyOneFruit(t, isApple, 'Apple') --------------------------------
    ...
    t.addConstraint(() => {
        const fruits = t.getSprites(s => s.visible && isApple(s));
    ...
    // -------------------------------- BEGIN INLINED CALL to testFruitSpawn(t, isFruit, fruitName, true) --------------------------------
    ...
    // -------------------------------- END INLINED CALL to testFruitSpawn(t, isFruit, fruitName, true) --------------------------------

    // -------------------------------- END INLINED CALL to testOnlyOneFruit(t, isApple, 'Apple') --------------------------------
};

const testOnlyOneBanana = async function (t) {
    // await testOnlyOneFruit(t, isBanana, 'Banana');

    // -------------------------------- BEGIN INLINED CALL to testOnlyOneFruit(t, isBanana, 'Banana') --------------------------------
     ...
     * Or if two fruits are visible, check that they have the exact same position. */
    t.addConstraint(() => {
        const fruits = t.getSprites(s => s.visible && isBanana(s));
    ...
    // -------------------------------- END INLINED CALL to testOnlyOneFruit(t, isBanana, 'Banana') --------------------------------
};
...
const testAppleGameOver = async function (t) {
    ...
    t.resetKeyboard();
    const {banana, time, score} = getSpritesAndVariables(t, ['banana', 'time', 'score']);
    t.addConstraint(() => {
        for (const appleInstance of apple.getClones(true)) {
...
const testAppleGameOverMessage = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testAppleGameOver(t, true) --------------------------------
    ...
    // -------------------------------- END INLINED CALL to testAppleGameOver(t, true) --------------------------------
};
...
const testBananaGroundPoints = async function (t) {
    t.seedScratch('seed');
...
const testBananaGroundMessage = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to testBananaGroundPoints(t, true) --------------------------------
    ...
    // -------------------------------- END INLINED CALL to testBananaGroundPoints(t, true) --------------------------------
};
...
const testTimerGameOver = async function (t) {
    ...
    // -------------------------------- BEGIN INLINED CALL to assertGameOver() --------------------------------

    const {banana, score} = getSpritesAndVariables(t, ['banana', 'score']);
    ...
    // -------------------------------- END INLINED CALL to assertGameOver() --------------------------------
