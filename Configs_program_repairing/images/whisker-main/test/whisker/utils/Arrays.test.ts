...
describe("Arrays", () => {
    ...
    test("Arrays.mapPartition of empty array gives pair of empty arrays", () => {
        const empty = [];
        const isEven = (x) => x % 2 == 0;
        const actual = Arrays.mapPartition(empty, isEven, isEven);
        const expected = [[], []];
        expect(actual).toStrictEqual(expected);
    });

    test("Arrays.mapPartition of non-empty array", () => {
        const numbers = [4, 3, 7, 8, 1, 2, 6, 5, 9];
        const myToString = (x) => "_" + x;
        const isEven = (x) => x % 2 == 0;
        const [even, odd] = Arrays.mapPartition(numbers, myToString, isEven);
        const [expectedEven, expectedOdd] = [["_4", "_8", "_2", "_6"], ["_3", "_7", "_1", "_5", "_9"]];
        expect(even).toStrictEqual(expectedEven);
        expect(odd).toStrictEqual(expectedOdd);
    });

    test("Arrays.count of empty array returns 0", () => {
        const empty = [];
        const isEven = (x) => x % 2 == 0;
        const actual = Arrays.count(empty, isEven);
        const expected = 0;
        expect(actual).toStrictEqual(expected);
    });

    test("Arrays.count of non-empty array", () => {
        const array = [2, 4, 5, 7, 6, 0, 1];
        const isEven = (x) => x % 2 == 0;
        const actual = Arrays.count(array, isEven);
        const expected = array.filter(isEven).length;
        expect(actual).toStrictEqual(expected);
    });

    test("Arrays.batches partitions the array in fixed-size iterable batches", () => {
        const array = [5, 4, 2, 7, 8, 6, 4, 3, 7, 8];
        const actual = [...Arrays.batches(array, 3)];
        const expected = [[5, 4, 2], [7, 8, 6], [4, 3, 7], [8]];
        expect(actual).toEqual(expected);
    });

    test("Arrays.removeIf", () => {
        const array = [2, 4, 5, 7, 6, 0, 1];
        const isEven = (x) => x % 2 == 0;
        const isOdd = (x) => !isEven(x);
        const expected = array.filter(isOdd);
        Arrays.removeIf(array, isEven);
        expect(array).toStrictEqual(expected);
    });

    test("Arrays.removeAll", () => {
        const array = [{blub: "test"}, [1, 2], ["a", {}], {blub: "test"}, {blubber: "test"}];
        const actual = Arrays.removeAll(array, {blub: "test"});
        expect(actual).toStrictEqual([[1, 2], ["a", {}], {blubber: "test"}]);
    });
    ...
    describe("Arrays.zip", () => {
        ...
        it("zips the elements of both arrays", () => {
            const expected = [[1, 't'], [2, 'e'], [3, 's'], [4, 't']];
            const actual = Arrays.zip(array1, array2);
            expect(actual).toStrictEqual(expected);
        });

        it("truncates the longer array", () => {
            const expected = [[1, 't'], [2, 'e'], [3, 's'], [4, 't']];
            const actual = Arrays.zip(array1, [...array2, 5, 6]);
            expect(actual).toStrictEqual(expected);
        });

        it("does not modify the input arrays", () => {
            const copy1 = deepCopy(array1);
            const copy2 = deepCopy(array2);
            Arrays.zip(array1, array2);
            expect(array1).toStrictEqual(copy1);
            expect(array2).toStrictEqual(copy2);
        });

        it("returns a new empty array when one array is empty", () => {
            const empty = [];

            const actual1 = Arrays.zip(array1, empty);
            expect(actual1).toStrictEqual(empty);
            expect(actual1).not.toBe(empty);

            const actual2 = Arrays.zip(empty, array1);
            expect(actual2).toStrictEqual(empty);
            expect(actual2).not.toBe(empty);
        });
    });
});
