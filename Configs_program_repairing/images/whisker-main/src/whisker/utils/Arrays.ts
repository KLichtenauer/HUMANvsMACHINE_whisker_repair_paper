...
export default class Arrays {
    ...
    /**
     * Returns a pair of `array` elements with, first, all elements that satisfy the `predicate`, and second, all
     * remaining elements that do not.
     *
     * @param array the array to partition
     * @param predicate the predicate with which to partition
     * @return the partitioning
     */
    static partition<T>(array: T[], predicate: (x: T) => boolean): Pair<T[]> {
        const yes = [];
        const no = [];

        for (const x of array) {
            (predicate(x) ? yes : no).push(x);
        }
    ...
    /**
     * Applies a `mapping` function to all `array` elements, and returns a pair of mapped `array` elements with, first,
     * all elements that satisfy the `predicate`, and second, all remaining elements that do not.
     *
     * @param array the array
     * @param mapping the mapping function
     * @param predicate the predicate
     * @return the pair of mapped elements
     */
    static mapPartition<T, U>(array: T[], mapping: (x: T) => U, predicate: (x: T) => boolean): Pair<U[]> {
        const mapPart = (pair, x) => {
            const [yes, no] = pair;
            (predicate(x) ? yes : no).push(mapping(x));
            return pair;
        };
        const init = [[], []];
        return array.reduce(mapPart, init);
    }

    /**
     * Returns the number of `array` elements that satisfy the given `predicate`.
     *
     * @param array the array
     * @param predicate the predicate
     * @return the number of `array` elements that satisfy the `predicate`
     */
    static count<T>(array: T[], predicate: (x: T) => boolean): number {
        let sum = 0;
     ...
     */
    static zipWithIndex<T>(array: T[]): [T, number][] {
        return array.map((elem, idx) => [elem, idx]);
    }

    static zip<T, U>(array1: readonly T[], array2: readonly U[]): [T, U][] {
        const result = [];

        for (let i = 0, len = Math.min(array1.length, array2.length); i < len; i++) {
    ...
    static deepToString<T extends AnyObject>(array: T[]): string {
        return '[' + array.map((o) => JSON.stringify(o)).toString() + ']';
    }

    static batches<T>(array: T[], size: number): T[][] {
        ...
        for (let start = 0, end = size; start < array.length; start = end, end += size) {
