...
export interface BuilderResult {
    algorithm: SearchAlgorithm<RepairChromosome>,
    fitnessFunction: RepairFitnessFunction,
    initialVariant: RepairChromosome,
}
...
export class RepairAlgorithmBuilder extends EventEmitter {
    ...
    constructor(
        ...
        private readonly _evaluator: TestExecutor,
    ...
    public buildSearchAlgorithm(): BuilderResult {
        this._handleLogging();
        ...
        let algorithm: SearchAlgorithm<RepairChromosome> | null = null;

        switch (this._config.algorithm.kind) {
            case "onePlusOne": {
                algorithm = new OnePlusOneEA<RepairChromosome>();
                algorithm.setChromosomeGenerator(chromosomeGenerator);
                algorithm.setFitnessFunctions(new Map([[0, fitnessFunction]]));
                algorithm.setFitnessFunction(fitnessFunction);
                algorithm.setProperties({stoppingCondition} as OnePlusOneProperties<RepairChromosome>);
                break;
            }
            case "genProg": {
                algorithm = new GenProg();
                algorithm.setChromosomeGenerator(chromosomeGenerator);
                algorithm.setFitnessFunction(fitnessFunction);
                algorithm.setSelectionOperator(this._buildSelection());
                algorithm.setProperties({
                    stoppingCondition,
                    populationSize: this._config.algorithm.populationSize,
                    elitismSize: this._config.algorithm.elitismSize,
            ...
            case "random": {
                algorithm = new RepairRandomSearch();
                algorithm.setChromosomeGenerator(chromosomeGenerator);
                algorithm.setFitnessFunction(fitnessFunction);
                algorithm.setFitnessFunctions(new Map([[0, fitnessFunction]]));
                algorithm.setProperties({
            ...
            default:
                throw new Error(`Unhandled search algorithm of kind "${this._config.algorithm.kind}"`);
        }
