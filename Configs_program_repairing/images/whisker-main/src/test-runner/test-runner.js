...
/**
 * @typedef {("pass" | "fail" | "error" | "skip")} TestExecutionStatus
 */
...
function postProcessResults(test, result) {
    ...
    const {status, error, passedAssertions, passedAssumptions} = result;
    ...
    const serializableError = error === null ? null : {
        stack: error.stack,
    ...
    const coveredBlocks = Array.from(CoverageGenerator.getCoveredBlockIds());

    return {name, status, error: serializableError, coveredBlocks, passedAssertions, passedAssumptions};
}
...
class TestRunner extends EventEmitter {
    ...
    async runTests(vm, project, tests, modelTester, props, modelProps) {
        ...
        return [finalResults, csv, mutantPrograms, coveragePerTest];
    }
    ...
    _setRNGSeeds(seed, test, vm) {
        ...
        // Prioritise seeds set in the test object.
        if (test !== undefined && "seed" in test) {
            Randomness.setInitialSeeds(test.seed);
        }

        // Check if a seed was specified on the command line and set the RNG generators to that seed if present.
        else if (seed !== undefined && seed !== 'undefined' && seed !== "") {
    ...
    async _executeTest(vm, project, test, modelTester, props, modelProps, defaultTimeoutPerTest = 0) {
        ...
        if (test) {
            const registerPassedAssertion = (stack) => passedAssertions.push(stack);
            const registerPassedAssumption = (stack) => passedAssumptions.push(stack)
            assert.on(assert.ASSERTION_PASSED, registerPassedAssertion);
            assume.on(assume.ASSUMPTION_HELD, registerPassedAssumption);
            ...
            try {
                ...
                assert.removeListener(assert.ASSERTION_PASSED, registerPassedAssertion);
                assume.removeListener(assume.ASSUMPTION_HELD, registerPassedAssumption);
            }
        ...
        result.passedAssumptions = passedAssumptions;
