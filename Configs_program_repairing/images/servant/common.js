...
function getProjectsInScratchPath() {
    return getProjectsInPath(scratchPath);
}

function getProjectsInPath({path, isDirectory}) {
    if (!isDirectory) {
        return [path];
    }
...
async function withTmpDir(callback, prefix = "whisker-") {
    const tmpDirPrefix = path.join(fs.realpathSync(os.tmpdir()), prefix);
    let tmpDir = null;
    try {
...
async function getProjectJsonFromSb3(filePath) {
    const sb3 = await JSZip.loadAsync(fs.readFileSync(filePath));
    const projectJsonString = await sb3.file("project.json").async("string");
    return JSON.parse(projectJsonString);
}

/**
 * Reads the sb3 file from `scratchPath`, replaces the contents of its `project.json` with the new
 * `projectJson`, and writes a new sb3 file to the given `filePath`.
 * @return {Promise<void>}
 */
async function writeSb3(projectJson, filePath) {
    const p = typeof scratchPath === "string" ? scratchPath : scratchPath.path;
    const originalProject = await JSZip.loadAsync(fs.readFileSync(p));
    const newProject = originalProject.file("project.json", JSON.stringify(projectJson, null, 4));
    const destination = fs.createWriteStream(filePath);
    await pipeline(newProject.generateNodeStream({type: 'nodebuffer', streamFiles: true}), destination);
}
