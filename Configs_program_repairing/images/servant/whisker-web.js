...
const {consoleForwarded, headless, whiskerUrl, acceleration, seed} = require("./cli").opts;
...
async function openNewBrowser() {
    ...
    logger.info("Browser opened after", Date.now() - now, "ms");
    return browser;
}
...
async function openNewPage(browser, opts = {}) {
    const {reduceLogSpam = false} = opts;
    const page = await browser.newPage({context: Date.now()});
    return await configureWhiskerWeb(page, {reduceLogSpam});
}

async function configureWhiskerWeb(page, {reduceLogSpam = false, waitUntil = "networkidle0"} = {}) {
    const now = Date.now()
            ...
            switch (msg.type()) {
                case 'warning':
                    if (reduceLogSpam && (
                        ...
                        text.includes("WebGL: CONTEXT_LOST_WEBGL: loseContext: context lost") ||
                        text.includes("Could not find comment with id:")
                    )) {
                        break;
                    }

                    logger.warn('Forwarded:', text);
                    break;
                case 'log': {
                    if (reduceLogSpam && (
                        text.startsWith('Seeding the RNG to ') ||
                        text.startsWith('Seeding the Scratch-VM to ')
                    )) {
                        break;
                    }

                    logger.info('Forwarded:', text);
                    break;
                }
                case 'trace':
                ...
                default:
                    ...
                    if (reduceLogSpam &&
                        text.startsWith('MaxListenersExceededWarning: Possible EventEmitter memory leak detected')) {
                        break;
                    }
                    ...
                    logger.error('Forwarded:', text);
            }
        });
    }

    const response = await page.goto(whiskerUrl, {waitUntil});

    if (!response.ok()) {
        const msg = `Whisker Web responded with status ${response.status()} (${response.statusText})`;
        logger.error(msg);
    ...
    if (acceleration) {
        await page.evaluate((acceleration) => {
            document.querySelector('#acceleration-value').innerText = acceleration;
        }, acceleration);
    }

    logger.info("Whisker Web loaded after", Date.now() - now, "ms");
