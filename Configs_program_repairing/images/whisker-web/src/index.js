...
const _runTestsWithCoverage = async function (vm, project, tests) {
    if (testsRunning) {
        ...
        try {
            ...
            [summary, csvResults, mutantPrograms, coveragePerTest] = await Whisker.testRunner.runTests(vm, project, tests,
...
window.Whisker.abortTestRun = function () {
    Whisker.scratch.stop();
    Whisker.outputRun.clear();
    Whisker.outputLog.clear();
};

window.Whisker.runTestsForRepair = async function (timeoutPerTest = 0) {
    Whisker.abortTestRun();
    const project = await Whisker.projectFileSelect.loadAsArrayBuffer(0);
    return await _runTestsWithCoverage(Whisker.scratch.vm, project, Whisker.tests, Whisker.testRunner);
};
...
const buildVariant = async function () {
    const accelerationFactor = Number(document.querySelector('#acceleration-factor').value);

    Whisker.scratch.stop();
    Whisker.outputRun.clear();
    Whisker.outputLog.clear();
    Whisker.outputFl.clear();

    const project = Whisker.projectVariant ?
        ProjectChromosome.deserialize(Whisker.projectVariant).project.getArrayBuffer() :
        await Whisker.projectFileSelect.loadAsArrayBuffer();
    ...
    try {
        setTimeout((window) => {
            if (typeof window.messageServantCallback === 'function') {
                ...
                Whisker.testRunner.abort();
                window.messageServantCallback({projectVariant: null});
                return;
            }
            Whisker.outputFl.println('Variant built');
        }, 180000, window);
        const projectVariant = await ProjectChromosomeFactory.buildVariant(
            project, Whisker.testRunner, Whisker.scratch.vm, CoverageGenerator, Whisker.tests, Thread, {accelerationFactor}
        );
        ...
        if (typeof window.messageServantCallback === 'function') {
            if (!projectVariant) {
                ...
                const serlVariant = projectVariant.serialize();
                console.log('--- serlVariant:\n', serlVariant);
                window.messageServantCallback({projectVariant: serlVariant});
            }
        }
...
const initComponents = function () {
    ...
    Whisker.outputFl = new Output($('#output-fl')[0]);
...
const initEvents = function () {
    ...
    $('#build-variant').on('click', buildVariant);
    ...
    $('#toggle-repair').on('change', event => {
        if ($(event.target).is(':checked')) {
            $(event.target)
                .parent()
                .addClass('active');
            _showAndJumpTo('#repair-controls');
        } else {
            $(event.target)
                .parent()
                .removeClass('active');
            $('#repair-controls').hide();
        }
    });
