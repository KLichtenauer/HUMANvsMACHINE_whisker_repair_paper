...
} from "../search/SearchAlgorithmProperties";
...
import {FitnessImprovementStoppingCondition} from "../search/stoppingconditions/FitnessImprovementStoppingCondition";
...
export class WhiskerSearchConfiguration {
    ...
    constructor(dict: Record<string, (Record<string, (number | string)> | string | number)>) {
        ...
        if ('testSuiteType' in this._config && this._config['testSuiteType'] === 'dynamic') {
            ...
            Container.isNeuroevolution = true;
        } else if (this._config.repair) {
            Container.isNeuroevolution = false;
            this._setRepairProperties();
        } else {
    ...
    private _setRepairProperties(): RepairProperties<any> {
