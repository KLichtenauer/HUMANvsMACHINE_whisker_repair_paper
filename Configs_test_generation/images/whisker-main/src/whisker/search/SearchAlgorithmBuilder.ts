...
import {GeneticAlgorithmProperties, RepairProperties, SearchAlgorithmProperties} from "./SearchAlgorithmProperties";
...
import {GenProg} from "../../repair/algorithms/GenProg";
...
export class SearchAlgorithmBuilder<C extends Chromosome> {
    ...
    buildSearchAlgorithm(): SearchAlgorithm<C> {
        ...
        switch (this._algorithm) {
            ...
            case "neatest":
                ...
                break;
            case "genProg":
                searchAlgorithm = this._buildGenProg() as unknown as SearchAlgorithm<C>; // FIXME ugly type assertion
                break;
            case "random":
            default:
    ...
    private _buildGenProg() {
        const genProg = new GenProg();
        // FIXME ugly type assertions
        genProg.setProperties(this._properties as RepairProperties<any>);
        genProg.setChromosomeGenerator(this._chromosomeGenerator as ChromosomeGenerator<any>);
        genProg.setFitnessFunction(this._fitnessFunction as unknown as RepairFitnessFunction);
        genProg.setSelectionOperator(this._selectionOperator as Selection<any>);
        return genProg;
    }
