import {BlockID, isBlockID, isScratchBlock, ScratchBlock} from "./types/blocks/Block";
...
import {getInputKeys, getOpcode, supportsInput} from "./utils/Blocks";
import {isBooleanReporterBlock, isStringNumberReporterBlock} from "./types/blocks/shapes/Reporter";
import {motionBlockOpcodes, Opcode} from "./types/blocks/Opcode";
...
type ScratchBlockLike = ScratchBlock | Node;

function unwrap(b: ScratchBlockLike): ScratchBlock {
    if (isScratchBlock(b)) {
        return b;
    }
...
export function canConnect(parent: ScratchBlockLike | null, next: ScratchBlockLike | null): boolean {
...
export function canBeInput(parent: ScratchBlockLike, key: InputKey, input: ScratchBlock | Input): boolean {
    parent = unwrap(parent);
