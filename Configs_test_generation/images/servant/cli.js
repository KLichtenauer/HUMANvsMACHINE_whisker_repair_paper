...
class WhiskerSubCommand extends Command {
    ...
    requireConfigPath(defaultConfigPath) {
        return this.requiredOption(
            '-c, --config-path <Path>',
            'path to a configuration file (".json")',
            (configPath) => util.processFilePathExists(configPath, '.json'),
            relativeToServantDir(defaultConfigPath),
        );
    }

    optionNumberOfJobs() {
        return this.option(
            '-j, --number-of-jobs <Integer|"max">',
    ...
    optionOutputCoverage () {
        return this.option('-o, --live-output-coverage', 'print new coverage output regularly');
    }
...
const subCommands = [
    ...
    newSubCommand('generate')
        ...
        .requireScratchPath()
        .requireConfigPath('../config/mio.json')
        .requiredOption(
    ...
    newSubCommand('repair')
        .description('repair buggy projects using tests and other projects')
        .requireTestPath()
        .requireConfigPath('../config/repair/genProg.json')
        .optionNumberOfJobs()
        .requiredOption(
            '-s, --scratch-path <Path>',
            'path to Scratch project to repair (".sb3")',
            (path) => util.processFilePathExists(path, '.sb3'))
        .requiredOption(
            '-o, --outDir <Path>',
            'output directory for the repaired variant',
            (path) => util.processDirPathExists(path),
        )
        .option(
            '-i, --keep-intermediate',
            'whether to write intermediate variants to the outDir',
        )
        .option(
            '-f, --fix-space-path <Path>',
            'path to repair source(s)',
            (path) => util.processFileOrDirPathExists(path)),

    newSubCommand('fault-loc')
        .description('fault localization')
        .requireTestPath()
        .requireScratchPath()
        .requiredOption(
            '-m, --metrics <Metrics...>',
            'fault localization metrics to apply',
            (metric, metrics = []) => {
                if (!metrics.includes(metric)) {
                    metrics.push(...util.processFaultLocalizationMetric(metric));
                }
                return metrics;
            })
        .requiredOption(
            '-x, --subsume-negative-results',
            'whether to subsume negative test results',
            (b) => !!b,
        )
        .optionNumberOfJobs(),

    newSubCommand('disambiguate')
        .description('Disambiguate block IDs (overrides project)')
        .requireScratchPath(),
];
