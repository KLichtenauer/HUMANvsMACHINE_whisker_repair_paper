...
function processFaultLocalizationMetric(metric) {
    // eslint-disable-next-line node/no-extraneous-require
    const {metrics} = require("whisker-main/dist/node/whisker").Whisker

    const supportedMetrics = Object.keys(metrics);
    ...
    if (!supportedMetrics.includes(metric)) {
        throw new InvalidArgumentError(`Unknown metric "${metric}"`);
    }
...
module.exports = {
    ...
    processFaultLocalizationMetric,
    processNumberOfJobs,
